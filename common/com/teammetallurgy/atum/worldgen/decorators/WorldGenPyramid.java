...
public class WorldGenPyramid extends WorldGenerator {
	...
	public boolean generate(World world, Random random, int i, int j, int k) {
		...
		for(int y = -6; y < 10; y++) {
			for(int x = y; x <= width - y; x++) {
				for(int z = y; z <= depth - y; z++) {
					...
					// if(id == 0 || id == Blocks.atumSand.blockID)
		...
		/*
		 * if(world.isAirBlock(i+7+1, j+1, k+7)) { } placeLadders(world, i+7+1, j+1, k+7, 4); }else if(world.isAirBlock(i+7-1, j+1, k+7)) { placeLadders(world, i+7-1, j+1, k+7, 4); }else
		 * if(world.isAirBlock(i+7, j+1, k+7+1)) { placeLadders(world, i+7, j+1, k+7+1, 4); }else if(world.isAirBlock(i+7, j+1, k+7-1)) { placeLadders(world, i+7, j+1, k+7-1, 4); }
		 */
		return false;
	}
