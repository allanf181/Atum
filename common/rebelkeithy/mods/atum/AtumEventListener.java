   ...
   public boolean onBonemeal(BonemealEvent event) {
      if(event.world.isRemote) {
         ...
         if(id == AtumBlocks.palmSapling.blockID) {
            ((BlockPalmSapling)((BlockPalmSapling)AtumBlocks.palmSapling)).growTree(event.world, event.X, event.Y, event.Z, new Random());
            event.setResult(Result.ALLOW);
         }

         if(id == AtumBlocks.flax.blockID && event.world.getBlockMetadata(event.X, event.Y, event.Z) < 5) {
            ((BlockFlax)((BlockFlax)AtumBlocks.flax)).fertilize(event.world, event.X, event.Y, event.Z);
            event.setResult(Result.ALLOW);
         }
