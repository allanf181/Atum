...
public class AtumMapGenCaves extends MapGenBase {
	...
	protected void generateLargeCaveNode(long par1, int par3, int par4, Block[] par6ArrayOfByte, double par6, double par8, double par10) {
		this.generateCaveNode(par1, par3, par4, par6ArrayOfByte, par6, par8, par10, 1.0F + super.rand.nextFloat() * 6.0F, 0.0F, 0.0F, -1, -1, 0.5D);
	}

	protected void generateCaveNode(long par1, int par3, int par4, Block[] par6ArrayOfByte, double par6, double par8, double par10, float par12, float par13, float par14, int par15, int par16, double par17) {
		...
		for (boolean flag1 = random.nextInt(6) == 0; par15 < par16; ++par15) {
			...
			if (!var62 && par15 == k1 && par12 > 1.0F && par16 > 0) {
				this.generateCaveNode(random.nextLong(), par3, par4, par6ArrayOfByte, par6, par8, par10, random.nextFloat() * 0.5F + 0.5F, par13 - 1.5707964F, par14 / 3.0F, par15, par16, 1.0D);
				this.generateCaveNode(random.nextLong(), par3, par4, par6ArrayOfByte, par6, par8, par10, random.nextFloat() * 0.5F + 0.5F, par13 + 1.5707964F, par14 / 3.0F, par15, par16, 1.0D);
				return;
			}
					...
					for (j3 = l1; !flag2 && j3 < i2; ++j3) {
						for (int d12 = l2; !flag2 && d12 < i3; ++d12) {
							for (int i4 = k2 + 1; !flag2 && i4 >= j2 - 1; --i4) {
								k3 = (j3 * 16 + d12) * 128 + i4;
								if (i4 >= 0 && i4 < 128) {
									if (par6ArrayOfByte[k3] == Blocks.water) {
										flag2 = true;
									}
								...
								if (var63 * var63 + d13 * d13 < 1.0D) {
									for (int k4 = k2 - 1; k4 >= j2; --k4) {
										...
										if (d14 > -0.7D && var63 * var63 + d14 * d14 + d13 * d13 < 1.0D) {
											Block b0 = par6ArrayOfByte[j4];
											if (b0 == Blocks.grass) {
												flag3 = true;
											}

											if (b0 == AtumBlocks.BLOCK_STONE || b0 == AtumBlocks.BLOCK_SAND) {
												if (k4 < 10) {
													par6ArrayOfByte[j4] = Blocks.lava;
												} else {
													par6ArrayOfByte[j4] = null;
													if (flag3 && par6ArrayOfByte[j4 - 1] == AtumBlocks.BLOCK_SAND) {
														par6ArrayOfByte[j4 - 1] = super.worldObj.getBiomeGenForCoords(j3 + par3 * 16, k3 + par4 * 16).topBlock;
													}
												}
											}
										}
