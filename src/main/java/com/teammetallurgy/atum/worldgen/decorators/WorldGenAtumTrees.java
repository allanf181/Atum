...
public class WorldGenAtumTrees extends WorldGenerator {
	...
	public WorldGenAtumTrees(boolean par1) {
		super(par1);
	}
		...
		for (aint1[par5] = aint[par5]; j1 <= i1; ++j1) {
			...
			while (k1 <= i1) {
				...
				if (d0 > (double) (par4 * par4)) {
					...
					if (l1 != null && l1 != Blocks.leaves) {
						++k1;
					} else {
						if (par6 != Blocks.leaves) {
							this.worldObj.setBlock(aint1[0], aint1[1], aint1[2], par6);
						}
	...
	private void placeBlockLine(int[] par1ArrayOfInteger, int[] par2ArrayOfInteger, Block par3) {
		...
		if (aint2[b1] != 0) {
			...
			for (int k = aint2[b1] + b4; j != k; j += b4) {
				...
				this.worldObj.setBlock(aint3[0], aint3[1], aint3[2], par3, b5, 1);
			}
		}
	...
	private void generateLeafNodeBases() {
		...
		for (int[] aint = new int[] { this.basePos[0], this.basePos[1], this.basePos[2] }; i < j; ++i) {
			...
			if (this.leafNodeNeedsBase(k)) {
				this.placeBlockLine(aint, aint2, Blocks.planks);
			}
		}
	...
	private boolean validTreeLocation() {
		...
		boolean isValidSoil = i == AtumBlocks.BLOCK_SAND;
		if (!isValidSoil) {
			return false;
		} else {
