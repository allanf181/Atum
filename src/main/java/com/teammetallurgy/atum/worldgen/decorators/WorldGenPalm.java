...
public class WorldGenPalm extends WorldGenerator {
	...
	public boolean generate(World par1World, Random par2Random, int par3, int par4, int par5) {
		...
		boolean flag = true;
		Block id = par1World.getBlock(par3, par4 - 1, par5);
		if((id == AtumBlocks.BLOCK_SAND || id == AtumBlocks.BLOCK_FERTILESOIL || id == Blocks.dirt) && par4 >= 1 && par4 + l + 1 <= 256) {
			int i1;
			int j1;
			Block block;
			for(i1 = par4; i1 <= par4 + 1 + l; ++i1) {
				...
				for(int soil = par3 - b0; soil <= par3 + b0 && flag; ++soil) {
					for(j1 = par5 - b0; j1 <= par5 + b0 && flag; ++j1) {
						if(i1 >= 0 && i1 < 256) {
							block = par1World.getBlock(soil, i1, j1);
							if(block != null && !block.isLeaves(par1World, soil, i1, j1) && block != AtumBlocks.BLOCK_FERTILESOIL && block != Blocks.dirt && !block.isWood(par1World, soil, i1, j1)) {
								flag = false;
							}
						} else {
							flag = false;
			...
			if(!flag) {
				...
				if(par4 >= 256 - l - 1) {
					...
					for(j1 = 0; j1 <= l; ++j1) {
						block = par1World.getBlockId(par3, par4 + j1, par5);
						Block var23 = Block.blocksList[block];
						if(block == 0 || var23 == null || var23.isLeaves(par1World, par3, par4 + j1, par5)) {
							this.setBlockAndMetadata(par1World, par3, par4 + j1, par5, AtumBlocks.BLOCK_LOG, this.metaWood);
						}
					}
	...
	public void spawnLeaf(World par1World, int x, int y, int z) {
		...
		if(block == null || block.canBeReplacedByLeaves(par1World, x, y, z)) {
			this.setBlockAndMetadata(par1World, x, y, z, AtumBlocks.BLOCK_LEAVES, this.metaLeaves);
		}
