...
public class ClientTickHandler {
	...
	public void onUpdate(TickEvent.PlayerTickEvent event) {
		...
		if (event.type.equals(event.type.PLAYER) && event.phase.equals(event.phase.START)) {
			...
			if (player.dimension == AtumIDS.DIMENSION_ID) {
				if (Minecraft.getMinecraft().gameSettings.renderDistanceChunks < (nightvision ? 1 : 2)) {
				...
				if (player.worldObj.isRaining()) {
					raining = true;
					if (Minecraft.getMinecraft().gameSettings.renderDistanceChunks < (nightvision ? 2 : 3)) {
						Minecraft.getMinecraft().gameSettings.renderDistanceChunks = nightvision ? 2 : 3;
					}
		...
		if (event.type.equals(event.type.RENDER) && event.phase.equals(event.phase.END)) {
