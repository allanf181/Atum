		...
		if(par1DamageSource.damageType == "player") {
			...
			if(!worldObj.isRemote) {
				...
				for(EntityPlayer player : players) {
					player.addChatMessage(this.getEntityName() + " was slain by " + slayer.getEntityName());
				}
			}
		}
		...
		for(int x = minX; x <= maxX; ++x) {
			for(int y = minY; y <= maxY; ++y) {
				for(int z = minZ; z <= maxZ; ++z) {
					...
					if(block != null) {
						if(id != AtumBlocks.BLOCK_LARGEBRICK && id != AtumBlocks.BLOCK_PHARAOHCHEST && Block.blocksList[id].isBlockSolid(worldObj, x, y, z, 0)) {
