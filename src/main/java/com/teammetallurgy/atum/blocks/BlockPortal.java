...
//			for(int z = -1; z < 2; z++) {
//				for(int y = -1; y < 1; y++) {
//					if(par1World.getBlockId(par2 + x, par3 + y, par4 + z) != Block.sandStone && par1World.getBlockId(par2 + x, par3 + y, par4 + z) != this && par1World.getBlockId(par2 + x, par3 + y, par4 + z) != Blocks.BLOCK_LARGEBRICK) {
	...
	public boolean tryToCreatePortal(World par1World, int x, int y, int z) {
		...
		for(int x1 = -1; x1 < 2; x1++) {
			for(int z1 = -1; z1 < 2; z1++) {
				par1World.setBlock(x + x1, y + 1, z + z1, AtumBlocks.BLOCK_PORTAL);
			}
		}
		return true;
	}
