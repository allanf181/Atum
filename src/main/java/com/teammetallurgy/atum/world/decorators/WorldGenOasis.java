                    ...
                    if (block != AtumBlocks.BLOCK_SAND) {
                        ...
                        for (i = 0; i < 10; ++i) {
                            ...
                            if (!var25 && block == AtumBlocks.BLOCK_FERTILESOIL) {
                                ...
                                int dz;
                                if (!var27 && block.canSustainPlant(world, par3 + var28, y, par5 + z, ForgeDirection.UP, (IPlantable) ((IPlantable) AtumBlocks.BLOCK_PAPYRUS))) {
                                    for (dx = -1; dx <= 1; ++dx) {
                                        for (dz = -1; dz <= 1; ++dz) {
                                            if (block.canSustainPlant(world, par3 + var28 + dx, y, par5 + z + dz, ForgeDirection.UP, (IPlantable) ((IPlantable) AtumBlocks.BLOCK_PAPYRUS))) {
