...
public class RenderGhost extends Render {
	...
	/**
	 * Returns a rotation angle that is inbetween two other rotation angles. par1 and par2 are the angles between which to interpolate, par3 is probably a float between 0.0 and 1.0 that tells us where
	 * "between" the two angles we are. Example: par1 = 30, par2 = 50, par3 = 0.5, then return = 40
	 */
	...
	protected void passSpecialRender(EntityLivingBase par1EntityLiving, double par2, double par4, double par6) {
		if(Minecraft.isGuiEnabled() && par1EntityLiving != this.renderManager.livingPlayer && !par1EntityLiving.isInvisibleToPlayer(Minecraft.getMinecraft().thePlayer) && (par1EntityLiving.getAlwaysRenderNameTagForRender() || ((EntityLiving) par1EntityLiving).hasCustomNameTag() && par1EntityLiving == this.renderManager.field_96451_i)) {
			float f = 1.6F;
