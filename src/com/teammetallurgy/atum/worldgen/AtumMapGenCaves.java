...
public class AtumMapGenCaves extends MapGenCaves {
	protected void generateLargeCaveNode(long par1, int par3, int par4, byte[] par5ArrayOfByte, double par6, double par8, double par10) {
		generateCaveNode(par1, par3, par4, par5ArrayOfByte, par6, par8, par10, 1.0F + this.rand.nextFloat() * 6.0F, 0.0F, 0.0F, -1, -1, 0.5D);
	}
	...
	protected void generateCaveNode(long par1, int par3, int par4, byte[] par5ArrayOfByte, double par6, double par8, double par10, float par12, float par13, float par14, int par15, int par16, double par17) {
		...
		for(boolean flag1 = random.nextInt(6) == 0; par15 < par16; par15++) {
			double d6 = 1.5D + MathHelper.sin(par15 * 3.141593F / par16) * par12 * 1.0F;
			double d7 = d6 * par17;
			...
			if((!flag) && (par15 == k1) && (par12 > 1.0F) && (par16 > 0)) {
				generateCaveNode(random.nextLong(), par3, par4, par5ArrayOfByte, par6, par8, par10, random.nextFloat() * 0.5F + 0.5F, par13 - 1.570796F, par14 / 3.0F, par15, par16, 1.0D);
				generateCaveNode(random.nextLong(), par3, par4, par5ArrayOfByte, par6, par8, par10, random.nextFloat() * 0.5F + 0.5F, par13 + 1.570796F, par14 / 3.0F, par15, par16, 1.0D);
				return;
			}
	...
	protected void recursiveGenerate(World par1World, int par2, int par3, int par4, int par5, byte[] par6ArrayOfByte) {
		...
		for(int j1 = 0; j1 < i1; j1++) {
			...
			if(this.rand.nextInt(4) == 0) {
				generateLargeCaveNode(this.rand.nextLong(), par4, par5, par6ArrayOfByte, d0, d1, d2);
			...
			for(int l1 = 0; l1 < k1; l1++) {
				...
				generateCaveNode(this.rand.nextLong(), par4, par5, par6ArrayOfByte, d0, d1, d2, f2, f, f1, 0, 0, 1.0D);
			}
		}
	}
	...
	private boolean isTopBlock(byte[] data, int index, int x, int y, int z, int chunkX, int chunkZ) {
		...
		return data[index] == Block.grass.blockID;
	}
	...
	protected void digBlock(byte[] data, int index, int x, int y, int z, int chunkX, int chunkZ, boolean foundTop) {
		...
		if((block == AtumBlocks.BLOCK_STONE.blockID) || (block == filler) || (block == top)) {
			if(y < 10) {
