...
public class ItemScarab extends Item {
	...
	public boolean onItemUse(ItemStack par1ItemStack, EntityPlayer par2EntityPlayer, World par3World, int x, int y, int z, int par7, float par8, float par9, float par10) {
		...
		} else if(blockID == AtumBlocks.BLOCK_LARGEBRICK.blockID) {
			block = AtumBlocks.BLOCK_LARGEBRICK;
		}
		if(block != null) {
			if(!((BlockPortal) AtumBlocks.BLOCK_PORTAL).tryToCreatePortal(par3World, x, y, z, block)) {
				if(par2EntityPlayer.capabilities.isCreativeMode) {
					for(int x1 = -2; x1 < 3; x1++) {
						for(int z1 = -2; z1 < 3; z1++) {
							par3World.setBlock(x + x1, y, z + z1, block.blockID);
						}
					}
					for(int x1 = -2; x1 < 3; x1++) {
						for(int z1 = -2; z1 < 3; z1++) {
							if(x1 + x == x + 2 || z1 + z == z + 2 || x1 + x == x - 2 || z1 + z == z - 2) {
								par3World.setBlock(x + x1, y + 1, z + z1, block.blockID);
							}
						}
					}
						...
						for(int x1 = -2; x1 < 3; x1++) {
							for(int z1 = -2; z1 < 3; z1++) {
								if((x1 + x == x + 2 && z1 + z == z + 2) || (x1 + x == x - 2 && z1 + z == z + 2) || (x1 + x == x + 2 && z1 + z == z - 2) || (x1 + x == x - 2 && z1 + z == z - 2)) {
									par3World.setBlock(x + x1, y + y1, z + z1, block.blockID);
								}
							}
						}
					}
